...
async function downloadCrowdinSurveys(options = {}) {
  ...
  console.log('📥 Downloading Survey CSV Files from Crowdin')
  console.log('='.repeat(50))
  console.log(`📁 Target directory: ${surveysDir}`)
  console.log(`🔄 Force overwrite: ${force}`)
  console.log(`📦 Backup existing: ${backup}`)
  console.log('')
  ...
  try {
    ...
    if (filesToProcess.length === 0) {
      console.log(`❌ No survey files found matching: ${survey}`)
    ...
    for (const filename of filesToProcess) {
      ...
      try {
        ...
        if (!validation.valid) {
          console.log(`   ❌ Invalid CSV: ${validation.error}`)
          results.push({
            file: filename,
            status: 'error',
        ...
        console.log(`   ✅ Created: ${validation.rows} rows, ${validation.languages} languages, ${validation.size}`)
        results.push({
          file: filename,
          status: 'success',
    ...
    if (successful > 0) {
      ...
      results.filter(r => r.status === 'success').forEach(result => {
        console.log(`   • ${result.file}: ${result.rows} translations, ${result.languages} languages`)
      })
    }
    ...
    console.log(`\n${success ? '🎉' : '⚠️'} Download ${success ? 'completed successfully' : 'completed with issues'}!`)

    return { success, results }

  } catch (error) {
    console.error(`❌ Failed to download combined CSV: ${error.message}`)
    return {
      success: false,
      results: [{
