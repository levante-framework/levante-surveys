...
describe('Survey JSON Files E2E Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Wait for the page to load and check if the Vue component is present
    cy.get('.survey-test-page', { timeout: 10000 }).should('be.visible')
    
    // Wait longer and debug what's available on window
    cy.window().then((win) => {
      console.log('Available window properties:', Object.keys(win).filter(k => k.includes('Survey') || k.includes('load')))
    })
    
    // Wait for the Vue component to mount and expose survey functions with longer timeout
    cy.window({ timeout: 10000 }).should('have.property', 'loadSurveyFromData')
    
    // Also check for Survey if available
    cy.window().then((win) => {
      if (!win.Survey && !win.loadSurveyFromData) {
        throw new Error('Neither Survey object nor load function is available')
      }
    })
  })
